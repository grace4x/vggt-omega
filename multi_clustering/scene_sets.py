...
keeps.
    ...
    python multi_clustering/scene_sets.py        # print the resolved scene sets
"""
...
DEFAULTS: dict[str, Source] = {
    # DL3DV's features are the ones `clustering/extract_features.py` already wrote
    # for the full 4288-scene dense train split: same model, same N_IMAGES, same
    # layers, and the 1466 wanted here are a subset of its rows. Pointing at that
    # file instead of a fresh npz saves ~5900 DINOv3 forward passes; `cluster.py`
    # selects the rows it needs by (subset, scene) and checks the metadata matches
    # ScanNet's extraction run.
    "dl3dv": Source(
        name="dl3dv",
        ...
        dense_only=True,
        limit=1466,
        features=REPO / "clustering" / "train_features_layers.npz",
    ),
