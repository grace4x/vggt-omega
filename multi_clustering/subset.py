...
`cluster.py` groups the pooled DL3DV + ScanNet scenes by visual mode. Sampling k (or a
...
of the data, without biasing toward a cluster's centre or its fringe.
    ...
    python multi_clustering/subset.py --k 5
    python multi_clustering/subset.py --pct 10
...
per_dataset = {}
for s in picked:
    # `dataset` is written by multi_clustering/cluster.py; a mapping from clustering/
