"""Cluster DL3DV and ScanNet scenes together with spherical k-means, on patch means.
...
`subset.py` can sample across modes instead of across datasets. The recipe is
`multi_clustering/cluster.py`'s -- per-layer DC removal, spherical k-means, in-cluster
dedup, then an ordering by within-cluster typicality -- but the vector each scene is
represented by is the concatenated per-layer *mean of the patch tokens*, not its CLS
token, so "similar" means similar overall frame content rather than a similar global
summary. See `extract_features.py` for why that is the descriptor of interest here;
an npz of CLS features is refused rather than clustered.
...
def load_pool(sources, split, num_frames):
    ...
    Returns (patch, rows) where `patch` is (N, n_images, dim) float32 -- the per-layer
    patch-token means, concatenated -- and `rows` carries the per-scene bookkeeping
    (dataset, subset, scene, first frame path) in the same order. Selection is by
    (subset, scene), not by row index, so an npz covering a superset of the wanted
    scenes is used as-is rather than re-extracted.
    """
    patch_parts, rows, recipe, models = [], [], None, set()
    for source in sources:
        wanted, _hw = resolve(source, split, num_frames)
        if not source.features.exists():
            raise SystemExit(
                f"missing {source.features}\n"
                f"  run: .venv/bin/python patch_avg_clustering/extract_features.py "
                f"--dataset {source.name}")
        d = np.load(source.features, allow_pickle=False)
        # A CLS npz (clustering/, multi_clustering/) has the same shape and metadata but
        # a different meaning, so cosine between the two would be quietly meaningless.
        if "patch" not in d.files:
            raise SystemExit(
                f"{source.features} has no `patch` array -- it holds CLS features, "
                f"not patch means; extract it with patch_avg_clustering/extract_features.py")
        ...
        here = (int(d["n_images"]), tuple(int(v) for v in d["patch_layers"]), bool(d["final_ln"]))
        if recipe is None:
            recipe = here
        elif here != recipe:
        ...
        patch_parts.append(d["patch"][take].astype(np.float32))
    ...
    dims = {c.shape[1:] for c in patch_parts}
    if len(dims) > 1:
    ...
    return np.concatenate(patch_parts), rows, (*recipe, next(iter(recorded), "unrecorded"))


def centered_unit(patch, n_layers, groups):
    """Per-layer DC removal, then L2-normalize each image's full vector.

    After `extract_features.py`'s per-layer L2-normalize, each layer's patch mean is
    ~90-99% a "DC" direction shared by every image regardless of content -- averaging
    the grid, if anything, makes that component larger than CLS's, since the
    frame-specific detail is what the mean cancels. Averaged into the
...
n_images, patch_layers, final_ln, model = recipe
...
print(f"  next: .venv/bin/python patch_avg_clustering/subset.py --k 5 --clusters {out.name}")
