"""One DINOv3 embedding per training scene, for clustering.

Same shape as the clustering-imgs script it is adapted from: take the first frame
of every scene, push it through DINOv3, keep the CLS token. The input here is the
...
# Per-patch tokens are 336 * 4096 * 2 B = 2.75 MB per scene, i.e. ~13 GB over the
# training set, and nothing downstream reads them -- cluster.py uses `cls` alone.
SAVE_PATCHES = False
...
if DENSE_ONLY:
    ...
    scenes = dense
frames = [sorted((DATA / e["path"] / "images").glob("*.jpg"))[0] for e in scenes]  # one frame per scene
...
for i, frame in enumerate(frames):
    ...
    cls.append(tokens[0].float().cpu().numpy())
    mean_patch.append(grid.mean((0, 1)).float().cpu().numpy())
    if SAVE_PATCHES:
        patches.append(grid.cpu().numpy())
...
np.savez(
    OUT,
    cls=np.stack(cls).astype(np.float16),                # (N, dim)
    mean_patch=np.stack(mean_patch).astype(np.float16),  # (N, dim)
...
print(f"wrote {OUT} ({len(scenes)} {SPLIT} scenes)")
